				...
				<div class="col-md-8">
					<div class="row">
						...
						<table border="1">
							...
							<th>数量</th><th>总金额</th><th>订单流水号</th><th>删除订单</th>
							</tr>
							<c:forEach items="${requestScope.selectorder_list }" var="order">
								<tr>
								<td>${order.name }</td>
								<td>${order.price }</td>
								<td>${order.detail }</td>
								...
								<td>
								<br/><br/>
								<a class="btn btn-warning" href="removeorder?good_id=${good.good_id}" role="button">X</a>                      		
                        		</td>
								</tr>
							</c:forEach>
							</table>
